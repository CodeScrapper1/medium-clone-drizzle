...
import { updateClapCount, updateReplyClapCount } from "@/actions/claps";
...
const ClapComponent = ({
  ...
  const ClapStoryorComment = async () => {
    ...
    try {
      if (!commentId) {
        const res = await updateClapCount(storyId);
        console.log(res, "res");
      } else {
        await updateReplyClapCount(storyId, commentId, type);
      }
      console.log("success");
    } catch (error) {
