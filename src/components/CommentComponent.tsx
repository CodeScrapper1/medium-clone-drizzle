"use client";
import { commentStory, getAllComments } from "@/actions/comments";
...
const CommentComponent = ({
  ...
  const CommentStory = async () => {
    try {
      await commentStory(storyId, Content, "");
...
const RenderComments = ({
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const result: any = await getAllComments(storyId, parentCommentId);
        if (result && result) {
          setComments(result);
        } else {
          console.log(result.error);
...
const UserEngagement = ({
  ...
  useEffect(() => {
    const fetchClapCountByUser = async () => {
      try {
        const claps: any = await ClapCountByUser(storyId, comment.id);
        setUserClaps(claps);
      } catch (error) {
...
const ReplyComments = ({
  ...
  useEffect(() => {
    const fetchClapCountByUser = async () => {
      try {
        const claps: any = await ClapCountByUser(storyId, parentCommentId);
        setUserclaps(claps);
      } catch (error) {
        console.log("Error fetching the user claps");
      }
    ...
    const fetchTotalClaps = async () => {
      try {
        const claps: any = await ClapCount(storyId, parentCommentId);
        setTotalClaps(claps);
      } catch (error) {
